...
const SingleStudentPost = ({ route }) => {
  ...
  const getAllComments = () => {
    getCommentsById(
      authToken?.token,
      `/blog/user/${itemData?._id}/allComments/alumni`
    )
  ...
  const handleComment = () => {
    if (comment === "") {
      ...
      COMMENT_IN_USER_BLOG_API(authToken?.token, itemData._id, comment)
        .then((res) => {
          console.log(res);
...
export default SingleStudentPost;
