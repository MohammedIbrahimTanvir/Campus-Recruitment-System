...
const SingleAlumniPostPage = ({ route }) => {
  ...
  const getAllComments = () => {
    getCommentsById(
      authToken?.token,
      `/blog/alumni/${itemData?._id}/allComments/user`
    )
  ...
  const handleComment = () => {
    if (comment === "") {
      ...
      ALUMNI_COMMENT_BLOG_API(authToken?.token, itemData._id, comment)
        .then((res) => {
          console.log(res);
...
export default SingleAlumniPostPage;
